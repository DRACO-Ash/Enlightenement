...
| A pool in which every candidate refuses spends a bounded budget and then says the budget was spent. `serve` re-raised the last item's refusal on its final attempt, which made the budget error unreachable and told the operator "one item is broken" when the fact was "four candidates refused". The budget's VALUE is asserted against a literal, because the first version of that assertion compared against the constant under test and widening the budget to eight left it green | `training/drill.py` | `test_a_refusing_pool_raises_the_budget_error_after_exactly_the_allowed_attempts` |

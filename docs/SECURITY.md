...
| Every content-supplied string on an unauthenticated route is bounded PER ENTRY AND capped in count. **FOUR surfaces carried this fault, not three, and V0.26.6 recorded the class as closed while the fourth was live in the file it edited, 110 lines from the fix - a 86,317-byte manifest response, LARGER than the one that release cites as the defect it shut.** The anonymous content 503 capped the error LIST at twenty and not the errors, so twenty errors of 4,253 characters were an 85,151-byte response; `/api/v1/content/manifest` served the same errors from the same load with no per-entry bound at all; the manifest capped `stimulus_params_unread.params` at twenty-five entries and not the NAMES, so a 500-character authored key was served verbatim; and the withhold reason was capped while the item id beside it was not. A count cap and a length cap are different bounds and neither substitutes for the other - and all four count caps were themselves unheld until V0.26.7, because every test poisoned exactly as many rows as the cap admits | `training_api.py`, `training/drill.py` | `test_an_anonymous_content_503_is_bounded_per_error_and_not_only_in_count`, `test_an_authored_param_name_is_bounded_on_the_anonymous_manifest` |

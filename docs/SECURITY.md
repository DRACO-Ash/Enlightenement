...
| A rendered stimulus never contradicts the answer key it is served with. The renderers read a vocabulary they had invented while the content authored another, so DRL-0034 drew the opposite of its own key and marked a correct reading wrong. What a renderer honours is now declared, what it does not is counted on the manifest, and the whole 140-item bank is rendered and checked against an agreement table | `generators/base.py`, `generators/products.py` | `test_no_rendered_stimulus_contradicts_its_own_authored_scene`, `test_the_unread_parameter_census_does_not_regress`, `test_a_numeric_item_resolves_the_value_it_will_be_scored_against` |
...
| Content-supplied strings are length-bounded before they reach the progress file. `extra="allow"` is the deliberate reversal that lets the package load unedited; the residual was length, and a 5000-character `version` was stored verbatim on a run row in a file read whole on every request | `training/drill.py` | `test_run_history_is_capped_so_the_file_cannot_grow_without_limit` |

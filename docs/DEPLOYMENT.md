...
| Version | 0.26.34, matching `pyproject.toml` and `src/enlightenment/__init__.py` |
...
- [x] Verification loop green (`sh scripts/verify.sh`), 1,010 passed and 2 skipped, coverage 97.82%
- [x] Pipeline simulation green against the version being shipped (`sh scripts/simulate-pipeline.sh 0.26.34`; with no argument the script defaults to 0.1.0 and would simulate a zip that is not the one going up)

...
**Approver:** Adam Field, Managing Director.
...
**Accuracy measures.** Every run records the exact content version hash it was scored under, so a
score is always interpretable against the procedure that produced it. The scoring engine decomposes
every score into which rule fired, on which evidence, against which procedure version, so an
operator can see and challenge the basis of a result. Competency estimates carry confidence
intervals and are never presented as a bare number.
...
| Adam Field, Managing Director | Approver. Not yet consulted on this draft |
...
| R2 | Score decomposition records which rule fired on which evidence against which procedure version. Content version hash on every run | ■ **Scorer validation against expert human raters on a held-out set, as a gate before any operator is scored.** ■ Confidence intervals never dropped in presentation. ■ Challenge route in the interface. **None built yet; no scoring engine exists.** | **Medium** with the gate enforced; **High** without it |
...
| R8 | None | ■ **A defined retention period and an implemented ageing job** for detailed run artefacts, with aggregate competence retained separately. **Neither set nor built.** | **Low** once set and built |
...
| Approved by | Adam Field, Managing Director. **Not yet signed** |

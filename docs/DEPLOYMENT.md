...
| App type | Web App |
| Version | 0.24.0, matching `pyproject.toml` and `src/enlightenment/__init__.py` |
...
- [x] Verification loop green (`sh scripts/verify.sh`), 899 passed and 2 skipped, coverage 96.67%
- [x] Pipeline simulation green against the version being shipped (`sh scripts/simulate-pipeline.sh 0.24.0`; with no argument the script defaults to 0.1.0 and would simulate a zip that is not the one going up)

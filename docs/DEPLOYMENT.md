...
| App type | Web App |
| Version | 0.23.18, matching `pyproject.toml` and `src/enlightenment/__init__.py` |
...
- [x] Pipeline simulation green against the version being shipped (`sh scripts/simulate-pipeline.sh 0.23.18`; with no argument the script defaults to 0.1.0 and would simulate a zip that is not the one going up)

...
| A rendered stimulus does not contradict its authored scene on any of the agreements the table names, and every one of the 140 drills is rendered and checked against them. **Narrowed at V0.24.2:** the row previously claimed a stimulus NEVER contradicts its key, which the table cannot prove - it covers seven agreements and skips silently where a fact is absent, and two counter-examples outside it were found within a day (an invisible burn the item was scored against, and a boolean drift onset read as a fraction, both now fixed and both now asserted). The census of authored parameters no renderer reads is the honest measure of the remainder, and it is 5 of 140 | `generators/base.py`, `generators/products.py` | `test_no_rendered_stimulus_contradicts_its_own_authored_scene`, `test_the_unread_parameter_census_does_not_regress`, `test_every_declared_read_actually_changes_the_surface`, `test_a_manoeuvre_is_visible_in_the_track_it_is_counted_from`, `test_a_burned_track_carries_no_duplicate_vertex` |
...
| The speed bonus is decided by the server's own clock. `elapsed_ms` arrives in the submission body, and a client posting zero collected the bonus every time while the server's `served_at` was recorded and never read: a score derived from a value the client asserts is a client-side control, which this project's hard rules forbid | `training/drill.py` | `test_the_speed_bonus_is_decided_by_the_server_clock_not_the_client_s` |
...
| A content-supplied count cannot make an unauthenticated response arbitrarily large. `obs_count: 18000` was read as a headcount and produced 18,000 tracks, 2.6 million points and 159 MB of JSON from one anonymous GET, which is a larger availability surface than the unbounded pending map closed in the same release. Every drill in the library is now served and measured against a stated byte budget | `generators/products.py`, `training/drill.py` | `test_every_served_stimulus_stays_inside_a_stated_payload_budget` |

...
| Every shortened identifier goes through ONE function, at every wire and every storage sink. `_bounded` truncates without a marker, so any two ids sharing a prefix longer than the cap collapse to one string - a fabricated identifier on an operator-facing surface, which this project forbids outright, and a comparison that silently fails. **SEVEN live instances, each surviving the fix for the last, and the sixth outside the module a scoped sweep had cleared:** the withheld map's key defeated selection; `RunRecord.item_id` was stored shortened and compared raw, so the attempt count was permanently zero for a long id and three attempts drew ONE seed against a docstring promising a seed per attempt; `due_items` served three distinct authored ids as one name; and the served drill payload's own `item_id` did the same, found only when a hostile tree was reshaped so its ids differ PAST the cap rather than before it. `served_identifier` appends a digest of the whole string, so shortened ids stay distinct and none reads as one an author wrote; a competency NAME stays on the plain bound, because prose is not an identity | The sixth named an authored identifier in the anonymous `document_too_large` 503 through a cross-module import of a PRIVATE helper, which this codebase's own docstring forbids while the public one already existed: two procedure ids sharing an 85-character prefix were refused under one name matching neither. The SEVENTH was the first outside `training/`: `content/loader.py` composed a raw id into a validation error and the composite was then cut at 256 on two anonymous surfaces, so two distinct authored ids served one identical string naming neither, with the `location` and `msg` - the only actionable part - truncated away. `content/` cannot import from `training/`, so the function now lives in `enlightenment.identifiers`, importable by every layer: **a rule a layer is unable to obey is not a rule, which is why six earlier sweeps each cleared the module they were scoped to.** `training_api` also passed a raw id to `log_event`, the one sink using no shortening at all, where `audit.py` cuts at 256 with no marker and no digest. **Measured on BOTH axes at all nineteen call sites, because neither mutation dominates the other:** a raw value is distinct and unbounded, so a distinctness assertion misses it; a truncation is bounded and collides, so a length assertion misses it. A line is held only when BOTH turn a test red, which means every served identifier needs a length assertion AND a distinctness assertion. Two earlier counts in this row were wrong because each was measured on one axis: four sites read as unheld under truncation alone, and two under raw alone. **All NINETEEN sites are held on both axes, with no standing exceptions.** Two rounds claimed otherwise: the last said two exceptions where four sites were in fact not red on both axes - `_serve_one`'s own name and the census key each carried a length assertion and no distinctness assertion, which is the failure mode this row's own rule names. And `RunRecord.procedure_id` and `.axis` are now driven rather than excused: they were argued to be write-only, and this register records that exact argument being made for `RunRecord.item_id` and being false, because write-only is a property of the current readers and not of a persisted field. **Every count in this row is measured per site on BOTH axes and recorded in the changelog**, because FOUR consecutive audit rows carried a false killed-by-its-own-test claim before this one - and the clause this replaces was one of them, left behind calling these two fields unheld in the same row that says they are driven - the last of them measured the list by inverting each site and still missed one, because the substituted mutation was a silent truncation while the state the code came from was a RAW value. An inversion has to return the code to where it was, not to something merely worse. The shortened form is also a PERSISTED format - `RunRecord.item_id` is written through it and compared on the way back - so it is pinned by a golden-value test after V0.26.15 changed it from 63 to 64 characters while describing the change as a relocation. | `training/drill.py`, `training_api.py` | `test_two_oversized_library_documents_are_named_distinctly_in_the_refusal`, `test_two_long_ids_produce_two_distinct_log_lines_and_two_distinct_load_errors`, `test_the_answer_route_logs_two_distinct_names_for_two_long_ids`, `test_the_shortened_identifier_form_is_pinned_because_it_is_persisted`, `test_the_unbuilt_product_log_line_names_two_long_ids_distinctly`, `test_a_serve_time_refusal_names_two_prefix_sharing_ids_distinctly`, `test_the_persisted_run_labels_are_bounded_and_distinct`, `test_a_long_item_id_still_gets_a_fresh_seed_on_every_attempt`, `test_two_long_item_ids_do_not_collapse_into_one_on_the_manifest` |
...
| No anonymous JSON route serves a content-sized body, held by enumerating the ROUTE TABLE, asserting the enumeration, and driving the STATE a stateful route serves from. **NINE surfaces across seven releases, and the sweeps written to end the class were themselves four of them.** The V0.26.8 sweep filtered on `GET`, so the anonymous `POST /api/v1/drill/answer` served 201,084 bytes unseen; it filtered on an unparameterised path, so the library routes served 2,497,065 and 342,884 bytes unseen; and its enumeration could be narrowed to one route without turning red. The V0.26.9 sweep then measured `/api/v1/drill/next` on a single draw, because that route is idempotent until answered - eight of the first twenty-one items exceeded its ceiling, to 145,130 bytes - and its rebuild silently DELETED the two count-cap assertions it inherited, so both caps went back to surviving inversion while this row went on citing them. The control now compares discovered routes against an exact set over the whole route table, calls each with resolved parameters and a valid body, asserts status before size, refuses an empty measurement, drives serve-then-answer across the item space, and splits the diagnostic ceiling from `MAX_PAYLOAD_BYTES` rather than asserting one number over both. Library documents FAIL CLOSED on a 64 kB budget rather than being field-bounded, because the flight plan makes that library an anonymous reference a per-field cap would mutilate; a companion assertion proves the shipped library still serves. **Scoped honestly: this holds the routes the app declares, on the fields this hostile tree poisons, and every exclusion is named with a reason rather than made by a filter.**  The ninth: `items_without_a_resolvable_answer` and `withheld_reasons` were per-entry bounded and uncapped in COUNT, so their size was set by the drill count and by accumulated runtime withhold state - 140 drills served 17,014 bytes against this project's own 16 kB ceiling, 560 served 64,675, and the runtime path grew over the container's life because a serve-time refusal carries a 256-character reason rather than the 32-character load-time one. Every sibling field on that dict was already count-capped; these two were the odd ones out. Capped now, with the UNTRUNCATED TOTAL served beside them so a shortened list cannot read as the whole gap. | `training_api.py`, `training/drill.py` | `test_no_anonymous_route_serves_a_content_sized_body_on_a_hostile_tree`, `test_a_stateful_route_is_measured_across_the_item_space_not_on_one_draw`, `test_an_anonymous_content_503_is_bounded_per_error_and_not_only_in_count`, `test_an_authored_param_name_is_bounded_on_the_anonymous_manifest` |
...
| A validation refusal names the KEY and its DOMAIN, never the authored value that failed it. A refusal message becomes a withhold reason and reaches the unauthenticated manifest: the security gate set `newest_at` to a real accept string from DRL-0005's own key and the anonymous route served it back. Nothing scoreable travelled, because `newest_at` is a two-value layout flag, and nothing bound the message either. Structural identifiers - a generator name, a product id - are still named, because a typo in one is undiagnosable otherwise, and both are length-bounded on the way out | `generators/products.py` | `test_no_refusal_message_quotes_the_authored_value_that_caused_it` |

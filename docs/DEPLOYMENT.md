...
| App type | Web App |
| Version | 0.17.0, matching `pyproject.toml` and `src/enlightenment/__init__.py` |
...
- [x] Verification loop green (`sh scripts/verify.sh`), 508 passed and 1 skipped, coverage 98.71%
- [x] Pipeline simulation green against the version being shipped (`sh scripts/simulate-pipeline.sh 0.17.0`; with no argument the script defaults to 0.1.0 and would simulate a zip that is not the one going up)

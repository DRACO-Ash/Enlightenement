...
| Every shortened identifier goes through ONE function, at every wire and every storage sink. `_bounded` truncates without a marker, so any two ids sharing a prefix longer than the cap collapse to one string - a fabricated identifier on an operator-facing surface, which this project forbids outright, and a comparison that silently fails. **Four live instances, each surviving the fix for the last:** the withheld map's key defeated selection; `RunRecord.item_id` was stored shortened and compared raw, so the attempt count was permanently zero for a long id and three attempts drew ONE seed against a docstring promising a seed per attempt; `due_items` served three distinct authored ids as one name; and the served drill payload's own `item_id` did the same, found only when a hostile tree was reshaped so its ids differ PAST the cap rather than before it. `served_identifier` appends a digest of the whole string, so shortened ids stay distinct and none reads as one an author wrote; a competency NAME stays on the plain bound, because prose is not an identity | `training/drill.py` | `test_a_long_item_id_still_gets_a_fresh_seed_on_every_attempt`, `test_two_long_item_ids_do_not_collapse_into_one_on_the_manifest` |

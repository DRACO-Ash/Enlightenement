...
| **An impossible reported figure is reported verbatim and the panel draws what the object is actually doing.** DRL-0005 carries the real -22,900,000°/day ASTRA 1M artefact and its own `explain` says the object "has not moved unusually at all". Two earlier attempts at this row were both wrong about it. The header first read "drawn to scale", the negation of the clamp; the clamp that replaced it drew the track at whatever the panel could express, which at the shipped seed is -1.44°/day - not an illegible fallback but a PLAUSIBLE REAL DRIFT RATE, well inside the few degrees per day the owner's neighbourhood products show. So the product drew an ordinary drifter leaving its box under a header reporting an impossible figure, while the key said nothing had happened: the plot contradicted its own answer, on the one item whose entire lesson is that a number and a picture can disagree. The item authors the MECHANISM too - `epoch_gap_ms: 4` - and nothing read it, so the accepted answer "the epochs are too close together" was reachable only by elimination. Now every track holds station, the two element-set epochs are stated to the millisecond as the evidence, and the clamp survives only for an out-of-envelope figure whose mechanism the content does not name. No shipped item is in that case | `generators/products.py` | `test_the_astra_artefact_is_reported_verbatim_and_drawn_as_a_held_object`, `test_an_absurd_authored_rate_is_clamped_for_drawing_and_reported_verbatim` |
| **Every drift rate the library DRAWS is one the real GEO belt produces, and a station-kept object is not drawn motionless.** Across 140 items the only drift rate anything authored was the -22,900,000 artefact and every held track was drawn at exactly zero, so the trainer taught an operator to reject the impossible without once showing the ordinary - and rejecting a figure is a comparison against a distribution you are carrying. The envelope is taken from the neighbourhood products the owner supplied on 2026-09-10 and is held as ROUNDED BOUNDS, never as transcribed readings, so `docs/PLOT-REALISM.md`'s promise that no value from a live operational product reaches content or code still holds. Bound over the whole bank rather than a sample, because a sample cannot show an absent distribution | `generators/products.py` | `test_no_waterfall_draws_a_rate_the_real_belt_does_not_produce`, `test_a_held_track_carries_a_real_station_keeping_residual` |
| **The station-keeping residual is bounded by the box its own header states, and the artefact presentation requires the gap to EXPLAIN the figure.** Two bounds on CONTENT-supplied quantities reaching a served surface, both found by the engineering gate against V0.27.10. `held + residual * when` is unbounded in the window length: at the longest authorable span, 60 days, a held track ran from -3.775° to +1.707° under a header reading "-3.0° to +3.0° of the primary", so the product asserted a station-keeping box its own marks had left. `MAX_SPAN_DAYS` is 60 and the span is content-supplied, so the bound belongs where the value reaches the range rather than where somebody notices. The artefact branch, separately, fired on ANY positive epoch separation: an authored 100°/day with a gap of one whole day rendered as a division-by-almost-nothing artefact, its header offering two epochs twenty-four hours apart as the evidence. The guard is now the arithmetic the artefact IS - the rate times its gap recovers the longitude difference the two element sets held, and the branch fires only when that comes back inside the authored box. Asserted at the CEILING and in both directions, because the shipped spans are exactly where both were already true | `generators/products.py` | `test_a_held_track_stays_inside_the_box_its_own_header_states`, `test_an_epoch_gap_that_cannot_explain_the_figure_is_not_presented_as_an_artefact` |
| **One object's draws from the seeded stream do not depend on its neighbours, so a replay reproduces the surface an operator actually saw.** The station-keeping residual is drawn for EVERY object and used only for the held ones, deliberately: drawn only where it is used, a change in the drifter count shifts the stream for every object behind it, and the debrief redraws a different neighbourhood from the one that was scored. The rationale was a comment held by nothing - the gate mutated it and the whole suite stayed green. The gate's own first assertion for it passed under the mutant by arithmetic luck at the shipped seed, because shifting the stream changes which observations survive the drop-out and the survivor count moved back the other way, so the property is asserted where it is claimed: the stream POSITION at which each object begins drawing, one integer per object with no value pinned | `generators/products.py` | `test_an_objects_draws_do_not_depend_on_how_many_of_its_neighbours_drift` |
...
| No anonymous JSON route serves a content-sized body, held by enumerating the ROUTE TABLE, asserting the enumeration, and driving the STATE a stateful route serves from. A per-field assertion holds only the fields somebody thought of, and every narrowing so far was made by a filter rather than declared: filtering on `GET` hid an anonymous `POST`, filtering on an unparameterised path hid the library routes, measuring an idempotent route on a single draw hid every item after the first, and an enumeration asserting nothing about itself could be narrowed to one route without turning red. The control therefore compares discovered routes against an exact set over the whole route table, calls each with resolved parameters and a valid body, asserts status before size, refuses an empty measurement, drives serve-then-answer across the item space, and splits the diagnostic ceiling from `MAX_PAYLOAD_BYTES` rather than asserting one number over both. Library documents FAIL CLOSED on a 64 kB budget rather than being field-bounded, because the flight plan makes that library an anonymous reference a per-field cap would mutilate; a companion assertion proves the shipped library still serves. **Scoped honestly: this holds the routes the app declares, on the fields this hostile tree poisons, and every exclusion is named with a reason rather than made by a filter.** **The TENTH surface was the one route excluded from the enumeration, and both reasons given for excluding it were false.** `GET /api/v1/sessions` was recorded as token-gated, which is true of the writes and false of the read, and as governed by `storage.MAX_SESSIONS` "which their own tests hold", while `MAX_SESSIONS` appeared in the suite only in that sentence. Measured on the wire at the cap, every field inside its declared maximum and accepted with 201: **1,231,926 bytes of ASCII and 4,711,926 with astral characters from one unauthenticated request**, the second past `MAX_PAYLOAD_BYTES`. Capped at `MAX_SERVED_SESSIONS`, newest kept, with the untruncated total and a `truncated` flag beside it, and swept with a byte ceiling against a store driven to its cap in astral characters. **Ten surfaces across eight releases, and the sweeps written to end the class were themselves four of them; `docs/CHANGELOG.md` carries that history and this row does not.** **The ELEVENTH was the control-character rule holding one of the two write routes.** `test_a_control_character_is_refused_at_the_write_boundary` drove `POST` only while being named for the boundary: deleting `FreeText` from the three `SessionPatch` fields left the whole suite green at 1,011 passed, twice, while `PATCH` accepted NUL, BEL, escape, `U+2028` and `U+00A0` with 200, and `notes` at its 2,000 cap of NUL renders to 12,000 bytes a row, so the 25 SERVED rows measure **335,264 bytes** against 262,144 with the other two fields at their caps in astral characters, and **351,264** with all three in NUL, both on a planted 500-row snapshot at twelve character ids and twenty character timestamps. Rows WRITTEN moves the figure even though only 25 are served, because it sets `total`'s digit count and the `truncated` flag: 26 rows written measures 335,262 and 25 measures 335,263. The test now drives both routes on the same table and runs against a widened rate budget, so the LIMITER cannot answer for the boundary at nineteen accepted writes against a `WRITE_LIMIT` of twenty. The worst accepted character is also derived PER FIELD now: one winner chosen on `title` and applied to `notes`, which carries 8,000 of the roughly 9,280 rendered bytes in a row, returned the four-byte emoji with `notes` accepting a six-byte character and reported 90% of a ceiling the API could exceed. **Two figures published in the fixture docstrings, 61,114 and 235,114 bytes, reproduced on nothing and were stale by the same 150 bytes; both are re-measured and now carry the fixture that produces them.** **And correcting them produced two MORE wrong figures, caught by the engineering gate in the same release.** The listing serves `MAX_SERVED_SESSIONS` rows whatever is written, so ROWS WRITTEN is a variable of the measurement: it sets `total`'s digit count, `rev`'s digit count, and whether `truncated` renders `true` or `false`, one byte. Measured, 99 rows written serves 235,862 and 100 serves 235,864, the two bytes being one extra digit in each of `total` and `rev`. The astral worst case measures **235,862 bytes, 89.97%** at 30 rows written and twelve character ids, which is the fixture the LISTING-CAP test runs - the derived helper's only caller - and the one figure never published; the hostile-tree sweep is the planted 500-row fixture at **235,264**, and an earlier version of this row swapped the two, so the correction for a wrongly-attributed fixture attributed one wrongly; **237,162, 90.47%** at 30 rows and sixty-four character ids; **237,163** at exactly 25 rows and sixty-four character ids, the extra byte being `truncated: false`; and **235,264, 89.75%** on the planted 500-row snapshot at twelve character ids. The bare 237,163 published first named a fixture it does not hold on, and the variance was attributed to "a byte or two of microsecond width" - invented, and wrong in kind, since `isoformat()` emits six microsecond digits or omits them entirely, moving a row by 0 or 14 bytes. A byte figure published without its fixture is a figure nobody can reproduce, which is how six of them went stale in this file and its fixtures; the correction now names rows written, id width, timestamp width and the write path on every one. | `training_api.py`, `training/drill.py` | `test_no_anonymous_route_serves_a_content_sized_body_on_a_hostile_tree`, `test_a_stateful_route_is_measured_across_the_item_space_not_on_one_draw`, `test_an_anonymous_content_503_is_bounded_per_error_and_not_only_in_count`, `test_an_authored_param_name_is_bounded_on_the_anonymous_manifest`, `test_the_anonymous_session_listing_is_count_capped_and_reports_an_honest_total`, `test_a_planted_session_past_the_byte_ceiling_fails_the_read_closed`, `test_a_planted_non_json_float_fails_the_read_closed`, `test_a_control_character_is_refused_at_the_write_boundary` |
...
| The STORED SNAPSHOT gets the same boundary rule as the content tree, and an unreadable one fails closed rather than 500. `GET /api/v1/sessions` is anonymous by the decision at accepted risk 5 and called `store.load` with no handler, so every malformed shape the store already refuses reached an unauthenticated caller as a generic 500: not JSON, not UTF-8, top level not an object, nesting too deep to PARSE, a string that PARSES and then cannot be encoded, a `sessions` list holding a NON-OBJECT entry, and - found one release later again - nesting too deep to SERIALISE, which is a different bound from the parse one and was the half this row previously read as covered. **Two consecutive releases over-claimed completeness about this store**, so the enumeration is now scoped to the shapes listed rather than to the store, and the member shapes have their own parametrised test. **That last shape failed twice over:** `dict(entry)` raises `TypeError` rather than `ValueError`, so it escaped the mapping every caller relies on and reached an anonymous caller as a 500 with `/healthz` still 200; and a list of PAIRS is coerced by `dict()` into a session row NOBODY WROTE, served anonymously with an honest-looking `count` and `total`. A fabricated record on a read boundary, which no length cap or byte ceiling can see because the row is well-formed once coerced, and which breaks the hard rule against inventing data outright. The container was checked and its MEMBERS were not. The encodability check is why this row exists: the snapshot had no encodability check, so **"the snapshot is trusted stored state" and "the progress file is not" were two different answers to one question in adjacent modules**. `TrainingStore.load` runs the same walk as `content/loader` now and raises `ValueError` like every other malformed shape; the route answers 503 `store_unavailable` with the fault named, length-bounded, and no stored VALUE in it. A stored KEY is named, for the reason the content row gives - a JSON pointer to a key is the key - and here that key is either one of this project's own field names or one planted by the actor who already holds write access to the volume, so it teaches that actor nothing. The pointer goes through `sanitise_log_value` at the raise site, which bounds it and strips a newline before it can reach either the wire or a log line. Not reachable from the HTTP edge - a surrogate in a body is a generic 422 in every form - so the precondition is write access to the data volume, an actor this model puts out of scope. Closed anyway, because a route that 500s on its own data cannot be diagnosed from a screenshot. The dependency-free health paths are asserted to stay 200 alongside | `storage.py`, `app.py` | `test_an_unreadable_snapshot_fails_closed_on_the_anonymous_listing` |
...
| All three state-changing routes (`POST /api/v1/sessions`, `PATCH /api/v1/sessions/{id}`, `POST /api/v1/drill/answer`) EMIT an audit line naming the actor, which is the accountability control under a shared token. The drill line additionally carries NEITHER the submitted answer nor any score: the plan forbids a personal performance figure in a log line, and an operator's own words are performance data. **The submitted answer reaching NO sink at all - not the reveal, not `progress.json`, not a log line - is now bound rather than read.** That property is the whole justification for exempting `DrillAnswer.response` from the free-text control rule the session fields carry, and it rested on a reading of the matcher and the run row: `Match` returns a verdict plus the AUTHORED `note` and `why_wrong`, and `RunRecord` takes `classification=outcome.matched` with `first_action=""`. A reading is what a later change breaks in silence | `app.py`, `training_api.py`, `training/drill.py` | `test_local_anonymous_mode_allows_the_write_and_records_the_actor_as_anonymous`, `test_a_gated_write_emits_one_audit_line_naming_the_token_actor`, `test_every_accepted_answer_emits_one_audit_line_carrying_no_performance_data`, `test_a_submitted_answer_is_neither_echoed_nor_persisted` |
...
| A malformed `sessions` field is rejected on migration, CONTAINER and MEMBERS both. The container check alone let a `sessions` list holding a non-object through: `dict(entry)` then raised `TypeError` rather than `ValueError` and escaped the 503, and a list of pairs was coerced into a fabricated session row served on the anonymous listing. Both halves are parametrised now, the member test over six shapes including the pair-list coercion, because the `rev` sibling beside it had been driven over five shapes while this one drove exactly one string | `storage.py` | `test_migrate_rejects_a_malformed_sessions_field`, `test_migrate_rejects_a_sessions_entry_that_is_not_an_object` |
...
| **The procedure INDEX is an index, not thirteen inlined documents.** The library screen needed a list of procedures and no route served one, so it had nothing to render and shipped a filler line on every card. `GET /api/v1/content/procedures` is that list, and it is a NEW anonymous surface, which is the shape this register has closed eleven times: inline the documents and an index becomes a content-sized body. So it carries identifiers, a bounded purpose and a step COUNT, with the step lists left on the per-procedure route behind the document budget. Every entry is bounded on the way out by the conventions the sibling routes already use - `served_identifier` on the id, because two ids differing past the cap would otherwise merge into one entry the interface then keys on, and `capped` on the prose, because `content/models.py` sets no maximum on any authored string. Measured on the shipped tree: **5,226 bytes** against the 64 kB library ceiling. The route is declared to both route-table enumerations rather than discovered by them - the read-route review and the anonymous size sweep both failed the build until it was named, which is those controls working | `training_api.py` | `test_the_procedure_index_is_bounded_and_inlines_no_document` |
| **An authored field of the wrong SHAPE is a diagnosed 503, never a 500 and never a plausible wrong value.** `capped(value: Any, ...)` calls `str()`, which never fails, so a container-shaped authored field was SERVED as a Python repr: every library card rendered `['LEO', 'MEO', 'GEO', 'HEO']` - correctly bounded, correctly escaped, correctly wrong, and invisible to a green suite. V0.27.1 repaired that on `regime` and left `purpose` exposed one line above it, **and the repair introduced a worse fault**: slicing a mapping raises `KeyError: slice(None, 10, None)` and slicing a number raises `TypeError`, so ONE malformed record answered HTTP 500 with a 26-byte body and killed the index for all thirteen procedures. Measured by the deploy-gate on seven re-authored trees: two 500s and four silent wrongs, including `steps` counting a mapping's keys and a string's characters. Closed as a TYPE gate at the boundary rather than a third field-by-field repair - `_authored_text` takes `str` only, `_authored_sequence` takes `list` only, and anything else raises the 503 this module's contract already promises, naming the field and the TYPE but never the value, because an authored value has no declared maximum. The text gate admitted `int` and `float` in its first draft as "legitimate scalars", which let `purpose: 7` render as "7" on a card; the schema declares every guarded field a string, so a number is a shape fault like any other. **The hostile-tree sweep beside this is structurally blind to shape**: its `stretch` replaces string leaves and preserves every container by construction, so it certifies length and entry count and cannot express this class - which is why a shape-hostile table sits beside the length-hostile tree. `name` and `status` are DECLARED `str` on the model, so a container there is refused at LOAD by pydantic and never reaches the route; asserted separately, because "the route validates it" and "the model validates it" are different claims and the first is false for those two. The client carries the same guard as depth: a non-array `regime` renders the unknown marker rather than being iterated, because `for...of` over a string yields characters | `training_api.py`, `ui/app.js` | `test_a_misshapen_content_field_is_a_diagnosed_503_and_never_a_500`, `test_a_misshapen_declared_field_is_refused_at_load_not_by_the_route` |
| **No figure from a design mockup reaches a shipped surface.** The design handoff of 08 September says plainly that its element sets, percentages, ratings and streak counts are "synthetic display data chosen to look plausible ... not a content specification", and `CLAUDE.md` forbids inventing a figure in user-facing content outright. So the mockup's session number, drill rating, rank tier, rank target, calibration claim and run length are asserted ABSENT from both shipped files rather than trusted to have been left out, and every count on the new screens is filled from `/api/v1/me` or omitted. The check found three real carriers on its first run, one of them a live defect: the top bar shipped the static label `day streak` before the script replaced it, so a browser rendered a streak claim with no source for as long as the fetch took. The rank tier, rank target, day streak and fourteen-day activity strip have no source in the dashboard at all and are withheld; the chip and rank slots keep the mockup's treatment and carry real counts under truthful labels. The weakest-axis card is held separately, because it is the one place a zero could be dressed as a finding: it chooses over MEASURED axes only, so an axis with no attempts cannot be reported as the operator's weakness | `ui/index.html`, `ui/app.js` | `test_the_interface_carries_no_synthetic_figure_from_the_mockup`, `test_the_session_screen_reports_absence_rather_than_inventing_a_figure` |
| **Every ink the interface renders text in meets the WCAG 2.2 AA normal-text floor of 4.5:1, measured against the shipped markup.** `docs/DESIGN-BRIEF.md` claimed its contrast figures were "enforced by tests that read the shipped markup and the canvas palette"; a grep for every hex in that document over `tests/` returned NOTHING, so the claim was the only thing holding the palette - the failure mode this register keeps finding, where the claim is what stops anybody looking. Closing it mattered immediately, because adopting the 08 September handoff verbatim shipped two of its own tokens below the floor at the sizes they are used at: `--ink-ghost` on `--surface-4` measured **3.80:1** behind 11px seed strings and `--ink-count` on `--surface-nav` measured **3.16:1** behind a 10.5px counter. Neither is large text, which starts at 18.66px bold or 24px regular, so the 3:1 allowance does not apply. Both were lightened on constant hue and saturation to 4.50:1 and 4.51:1; the one pair the handoff asked to be verified, `--ink-faint` on `--surface-4`, measured 4.95:1 and was left alone. The test parses the tokens out of `:root` rather than duplicating them, so a token renamed or deleted fails it instead of silently dropping out of scope | `ui/index.html` | `test_every_shipped_ink_meets_the_contrast_floor` |
| **The interface is served at `/` as well as `/ui`, and the negotiation FAILS TOWARDS the machine-readable body.** `/` is part of the App Store health contract, and the console's "Open App" button opens it - so before V0.27.9 every human who followed that button landed on `{"name":"Enlightenment",...}` and reasonably read a green ten-stage deploy as a failed one. The split is on the caller's own `Accept` header and only a header that NAMES `text/html` gets the interface: no header, `*/*`, `application/json`, `text/plain` and every probe keep the JSON byte-for-byte, which is six of the nine table rows because the contract is what must not break. `text/html;q=0` is a client refusing HTML and is honoured as a refusal rather than as a mention. Both branches answer 200 with no `location` header, because the platform router reads a 302 at root as unhealthy - a redirect was the shorter fix and the wrong one. **The exemption from rate limiting is on the BRANCH, not on the path, and V0.27.9 got that wrong.** `/` is in `UNLIMITED_PATHS` because the platform probes it and a 429 there reads as unhealthy - reasoning that covers a dependency-free 58-byte JSON body, not the 34 kB document V0.27.9 put behind the same path. The security gate measured it: 400 unauthenticated `GET /` with `Accept: text/html` all answered 200 with no 429, a 589-fold body amplification on an unmetered path with `cache-control: no-store` forcing every request to the origin, while the identical resource at `/ui` was throttled correctly. The gate also reported that the file read starved the store's own thread-pool work, at 85.9x, and **withdrew that half of its own finding on re-review**: flooding `/livez` - plain text, no filesystem, no store - reproduces 65.4x of it, so the degradation was concurrent coroutines saturating a single-threaded event loop in the harness rather than the executor. The figure is recorded as withdrawn rather than quietly dropped, because it stood in this row for one release. Fixed at V0.27.13: the limiter meters the markup branch, which is the control that was actually missing. The interface files are also read once per process and HELD, but that is a performance optimisation on immutable image contents and NOT a security control, and saying otherwise would put a boundary where there is none. Only success is cached, so the 503 branch stays reachable. The branch test itself was over-broad for one release and is corrected at V0.27.14: it applied to all six exempt paths, so the five that cannot serve markup returned 429 under `Accept: text/html` once a bucket was spent - costing an operator the 503-plus-errno diagnosis in a browser at the moment they need it, while never reaching the kubelet, which sends no `text/html`. Safe for the health contract by construction and checked rather than asserted: every probe path and `/` with no header, `*/*`, `application/json` or `text/plain` takes the JSON branch, so the limiter can never 429 a kubelet. **The other security property is that the second path carries the SAME air gap**: one `interface_response` builds both, so `_UI_HEADERS` cannot diverge, and serving this markup from a route that had lost its Content-Security-Policy would remove the posture silently. Asserted on the policy that is actually present - `frame-ancestors 'none'`, not the legacy `X-Frame-Options`, which the first draft of the test asserted and which this response does not carry | `app.py`, `training_api.py` | `test_root_serves_the_interface_only_to_a_caller_that_asks_for_html`, `test_the_interface_is_byte_identical_at_the_root_and_at_its_own_path`, `test_root_returns_200_and_never_a_redirect`, `test_the_markup_branch_at_the_root_is_rate_limited_and_the_probe_branch_is_not`, `test_a_missing_interface_file_fails_closed_and_is_not_remembered_as_missing`, `test_probe_paths_are_never_rate_limited` |
...
## What the write boundary refuses in free text

Recorded because the refusal cannot say it. `title`, `scenario` and `notes` on both session write
routes answer a rejected character with one generic `422 {"error": "invalid request"}` that never
names it, by the boundary rule this project holds everywhere, and OpenAPI is disabled on this
build (`openapi_url=None`), so there is nowhere else an operator hitting that 422 can look.

The rule is `str.isprintable()`, plus `\n`, `\r` and `\t`. Measured at V0.26.36, rendered bytes
per code point under `json.dumps(..., ensure_ascii=False)`:

| Class | Verdict | Examples, with rendered bytes |
| --- | --- | --- |
| Letters, digits, marks, symbols, punctuation, ASCII space | accepted | `a` 1, `e-acute` 2, CJK 3, emoji 4, combining acute U+0301 2, Persian digit U+06F5 2, `°` 2, `−` 3 |
| Line break, carriage return, tab | accepted | `\n` 2, `\r` 2, `\t` 2 |
| C0 and C1 controls, anything else in Cc | refused | NUL 6, BEL 6, ESC 6, DEL 1, NEL U+0085 2 |
| Format characters, Cf | refused | ZWSP U+200B 3, ZWJ U+200D 3, LRM U+200E 3, RLO U+202E 3, BOM U+FEFF 3 |
| Non-ASCII space separators, Zs | refused | NBSP U+00A0 2, NNBSP U+202F 3 |
| Line and paragraph separators, Zl and Zp | refused | U+2028 3, U+2029 3 |
| Unpaired surrogates, Cs | refused EARLIER | U+D800; pydantic's unicode parse refuses it before this rule is reached, so the message differs |

**Two mechanisms, and the table says "refused" for both.** A character in this list reaches a 422
by one of two routes, and which one depends on where it sits in the value. Anywhere INSIDE a value
it hits the rule above and pydantic answers `value_error`. As the WHOLE value it may never reach
the rule at all: `str_strip_whitespace` removes Unicode White_Space first, so a `notes` field of
nothing but NEL, NBSP, NNBSP, U+2028, U+2029, vertical tab, form feed, U+1680, U+3000, U+205F or
U+2002 is **accepted with 201 and stored as the empty string**, while the same value in `title` or
`scenario` is 422 by `min_length` rather than by the rule. The strip set is Unicode White_Space
rather than Python's `str.isspace()`, which is why U+001C to U+001F are refused by the rule while
U+0085 and U+00A0 are stripped away before it. No security consequence, because empty is the
cheapest value there is, and recorded because this is the document that answers "why was my paste
rejected" and the answer differs by position. It is also the distinction the derived test fixture
now depends on: a character that strips away cannot fill a field at any length, so the fixture
requires the model to KEEP a run of the candidate before counting it accepted.

Two consequences worth stating plainly:

● **The refusal is not uniformly about size.** NUL at six rendered bytes is what motivated the
  rule, and it is the one that broke a served ceiling, but DEL costs one byte and NBSP two and
  both are refused. The rule is the printable class; size is why it was worth having. An earlier
  test asserted the converse, that every refused character costs more than the widest accepted
  one, and that assertion has been withdrawn as false rather than tightened.
● **Refusing Cf closes bidi spoofing as well**, so a Trojan-Source style RLO override cannot
  reach the anonymous session listing. That was not the reason for the rule and is recorded as a
  consequence rather than claimed as a design intent.

What it costs: text pasted from a document carrying a non-breaking space, a zero-width-joiner
emoji sequence, and Arabic or Persian joiners are all refused. That is accepted as the right side
to err on for fields that fill an anonymous byte ceiling; loosening it would put the ceiling back
in the hands of an escaping factor.

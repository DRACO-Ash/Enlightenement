...
| Every content-supplied string on an unauthenticated route is bounded PER ENTRY, not only in count. Three surfaces carried the same fault: the anonymous content 503 capped the error LIST at twenty and not the errors, so twenty errors of 4,253 characters were an 85,151-byte response; the manifest capped `stimulus_params_unread.params` at twenty-five entries and not the NAMES, so a 500-character authored key was served verbatim; and the withhold reason was capped while the item id beside it was not. A count cap and a length cap are different bounds and neither substitutes for the other | `training_api.py`, `training/drill.py` | `test_an_anonymous_content_503_is_bounded_per_error_and_not_only_in_count`, `test_an_authored_param_name_is_bounded_on_the_anonymous_manifest` |

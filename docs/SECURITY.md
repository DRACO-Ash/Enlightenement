...
| No anonymous JSON route serves a content-sized body, held by enumerating the ROUTE TABLE and asserting the enumeration. **SEVEN surfaces carried this class across five releases, each fix correct and each sweep incomplete.** The V0.26.8 sweep was itself three of them: it filtered on `GET`, so the anonymous `POST /api/v1/drill/answer` served 201,084 bytes unseen; it filtered on an unparameterised path, so the two library routes served 2,497,065 and 342,884 bytes unseen; and `assert paths` could not tell that the discovery had been narrowed to a single route, which a mutation proved by leaving it green. `/api/v1/drill/next` was excluded on a false rationale: `MAX_PAYLOAD_BYTES` governs the rendered stimuli only, and saw 2,904 bytes of a 206,027-byte body whose `prompt` was 200,085. The sweep now compares the discovered routes against an EXACT set, calls every one with resolved parameters and a valid body, asserts the status before the size, and refuses to pass on an empty measurement. Library documents are not field-bounded, because a per-field cap would mutilate a reference the flight plan makes anonymous by design; they FAIL CLOSED on a 64 kB document budget instead, and a companion assertion proves the shipped library still serves. **Scoped honestly: this holds the routes the app declares, on the fields this hostile tree poisons. An earlier tree listed a library file that does not exist and skipped it with `continue`, so the file list now comes from the loader's own manifest.** | `training_api.py`, `training/drill.py` | `test_no_anonymous_route_serves_a_content_sized_body_on_a_hostile_tree`, `test_an_anonymous_content_503_is_bounded_per_error_and_not_only_in_count`, `test_an_authored_param_name_is_bounded_on_the_anonymous_manifest` |

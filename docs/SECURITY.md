...
| No anonymous JSON route serves a content-sized body, held by ENUMERATING THE ROUTES rather than the fields. **This row made a universal claim three releases running while a surface was live, and the claim is what stopped anyone looking.** V0.26.3 bounded the withhold reason; V0.26.6 found three more and missed a fourth in the file it edited; V0.26.7 fixed that and missed two on `/api/v1/me`, where 20 raw due-item ids and 8 raw competency names measured 221,589 bytes with a team token configured and no `Authorization` header. Six surfaces in total, each fix correct and each sweep incomplete, because a per-field assertion can only hold the fields somebody thought of. The control now walks the app's own route table on a hostile tree and asserts a body ceiling, so a new route or a new raw content string fails without anyone noticing it first; explicit count and length assertions sit beside it, because a body ceiling loose enough for honest content cannot see a count cap that never bites. `/api/v1/drill/next` is excluded by name and governed by `MAX_PAYLOAD_BYTES`. **Scoped honestly: this holds the anonymous API GET routes the app declares, on the fields that tree poisons. It is not a proof that no unbounded string exists.** | `training_api.py`, `training/drill.py` | `test_no_anonymous_route_serves_a_content_sized_body_on_a_hostile_tree`, `test_an_anonymous_content_503_is_bounded_per_error_and_not_only_in_count`, `test_an_authored_param_name_is_bounded_on_the_anonymous_manifest` |

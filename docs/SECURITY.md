...
| The loaded library cannot be mutated by a caller: every content model is a FROZEN pydantic model rather than a mutable record handed out by copy. Stronger than the previous control, because a frozen model refuses the write instead of absorbing it into a copy nobody reads | `content/models.py` | `test_an_unmodelled_field_is_carried_rather_than_rejected` |
